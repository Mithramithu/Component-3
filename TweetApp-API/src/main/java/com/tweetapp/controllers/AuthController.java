...
public class AuthController {
	...
	public UserModel subscribeClient(@RequestBody UserModel userModel) {
		return userRepository.save(userModel);
	}

	
	/*
	 * try { UserModel savedUser = userModelService.createUser(userModel); return
	 * new ResponseEntity<>(savedUser, HttpStatus.CREATED); } catch
	 * (UsernameAlreadyExists e) { return new ResponseEntity<>(new
	 * AuthenticationResponse("Given userId/email already exists"),
	 * HttpStatus.CONFLICT); }
	 * 
	 * catch (Exception e) { return new ResponseEntity<>(new
	 * AuthenticationResponse("Application has faced an issue"),
